...
list = [
    ...
    nomove = value.createList("NoMove", ["Always", "OnlyOnGround", "Off"], "Off"),
        ...
        onPacket: function (e) {
            if (mc.thePlayer) {
                ...
                if (p instanceof C16PacketClientStatus) {
                    if (p.getStatus() == "OPEN_INVENTORY_ACHIEVEMENT") {
                        if (openInventory) e.cancelEvent();
                        else openInventory = true;
                        nowindowpackets.get() && e.cancelEvent();
                    }
                } else if (p instanceof C0DPacketCloseWindow || p instanceof S2EPacketCloseWindow) {
                    openInventory = false;
                    nowindowpackets.get() && p instanceof C0DPacketCloseWindow && e.cancelEvent();
                } else if (p instanceof S30PacketWindowItems) {
                    ...
                    if (p.getAction() == "ATTACK") {
                        if (~isEating) mc.gameSettings.keyBindUseItem.pressed = false, isEating = -1;
                        attackTimer.reset();
                        selectWeapon(e);
                    }
                ...
                } else if (p instanceof S24PacketBlockAction) {
                    if (playing && actions["Open Chests"].module.state && ignorelootedchests.get() && p.getBlockType() == Blocks.chest && p.getData2()) chestBlacklist.push(mc.theWorld.getTileEntity(p.getBlockPosition()));
                } else if (p instanceof S45PacketTitle) {
                    if (p.getMessage() && reopenrefilledchests.get()) {
                        var msg = p.getMessage().getUnformattedText();
                        if (msg.contains("refill") || msg.contains("reabastecidos")) chestBlacklist = [];
                    }
                } else if (p instanceof C03PacketPlayer) onGround = p.onGround;
            }
        },
...
var targetModule, chestBlacklist = [], openInventory = false, lookingAtChest, lastChest, receivedItems, throwingPotion, userWhitelist, userBlacklist, isEating = -1, sortValues, sortValues, sortTargets, onGround = false,
openTimer = new MSTimer(), stealTimer = new MSTimer(), invTimer = new MSTimer(), attackTimer = new MSTimer(), throwTimer = new MSTimer(), playAgainTimer = new MSTimer(), equipTimer = new MSTimer();
...
BLOCK_BLACKLIST_2 = [Blocks.anvil, Blocks.crafting_table, Blocks.enchanting_table];
...
Object.keys(actions).forEach(function (action) {
    module.push({
        ...
        values: [
            ...
            !["Select Weapons", "Play Again"].includes(action) ?
                (actions[action].delay = value.createInteger("NoAttackDelay", action == "Steal Items" ? 0 : 500, 0, 3000)) :
            action == "Select Weapons" ?
                (actions[action].preferSwords = value.createBoolean("PreferSwords", true)) :
            null
        ],
        onLoad: function () {
        ...
        module.slice(1).forEach(function (action) {
            if (action.module.state) {
                switch (action.name) {
                    ...
                    case "Steal Items": shownValues.push(gamedetection, stealdelay, startdelay, closedelay, modifylistings, whitelist, blacklist, nomove); break
                    case "Drop Garbage": shownValues.push(gamedetection, invopen, nowindowpackets, invdelay, modifylistings, whitelist, blacklist, nomove); break
                    case "Equip Armor": shownValues.push(gamedetection, invopen, nowindowpackets, equipdelay, modifylistings, whitelist, blacklist, nomove); break
                    case "Sort Hotbar": shownValues.push(gamedetection, invopen, nowindowpackets, invdelay, slot1, slot2, slot3, slot4, slot5, slot6, slot7, slot8, slot9, nomove); break
...
function parseValue (value) {
    VALUE_FIELD.set(value, value.get().replaceAll(";", ",").replaceAll(" ", "").replaceAll(",", ", "));
    updateSortValues();
    if (mc.thePlayer) mc.thePlayer.inventory.inventoryChanged = true;
}

function updateSortValues () {
    sortValues = SLOT_VALUES.map(function (e) e.get().split(", "));
...
function stealItems () {
    if (isReady("Steal Items") && mc.currentScreen instanceof GuiChest && checkNoMove()) {
...
function selectWeapon (e) {
    if (isReady("Select Weapons") && playing && !~isEating && !ScaffoldModule.state && !TowerModule.state) {
        var hotbarStacks = Java.from(mc.thePlayer.inventory.mainInventory).slice(0, 8),
            stacks = hotbarStacks.slice().sort(function (b, a) getAttackDamage(a) - getAttackDamage(b));
        if (~getAttackDamage(stacks[0])) {
            if (actions["Select Weapons"].preferSwords.get()) stacks[0] = stacks.find(function (s) s && s.getItem() instanceof ItemSword) || stacks[0];

            var targetSlot = hotbarStacks.indexOf(stacks[0]);
...
function selectBlocks () {
    if (isReady("Select Blocks") && !mc.thePlayer.openContainer.windowId && !openInventory && !mc.currentScreen && !ScaffoldModule.state && !TowerModule.state && !~isEating && mc.gameSettings.keyBindUseItem.isKeyDown()) {
        ...
        if (mc.objectMouseOver && mc.objectMouseOver.typeOfHit == "BLOCK" && (!stack || (!(stack.getItem() instanceof ItemBlock) && !(stack.getItem() instanceof ItemFood)))) {
...
function shouldManageInventory () mc.thePlayer.inventory.inventoryChanged && !~isEating && (invopen.get() == "Simulate" ? !mc.thePlayer.openContainer.windowId : invopen.get() != "Require" || mc.currentScreen instanceof GuiInventory) && checkNoMove();

function checkNoMove () (nomove.get() == "Off" || (nomove.get() == "OnlyOnGround" && !onGround)) || !MovementUtils.INSTANCE.hasMotion();
