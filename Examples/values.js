...
module = {
    ...
    author: "CzechHek",
    onEnable: function () {
        setValues(MathExamClass, question1);
        mistakes = 0;
    },
    onUpdate: function () {
        if (answer = (modulevalue = getValues(MathExamClass)[0]).get()) {
            modulevalue.set("");
            switch (modulevalue) {
                case question1.value:
                    parseInt(answer) != 2 && mistakes++;
                    setValues(MathExamClass, question2);
                    break
                case question2.value:
                    parseInt(answer) != 4 && mistakes++;
                    setValues(MathExamClass, question3);
                    break
                case question3.value:
                    parseInt(answer) != 48 && mistakes++;
                    setValues(MathExamClass, question4);
                    break
                case question4.value:
                    parseInt(answer) != 4 && mistakes++;
                    setValues(MathExamClass, value.createText("Number of mistakes", mistakes.toString()));
